...
class calc:
 ...
 def __init__(self,master):
  ...
  master.geometry(newGeometry='190x130')
  self.e = Entry(master)
  self.e.grid(row=0,column=0,columnspan=6,pady=3,sticky=(N, S, E, W))
...
#  Button(master,text="=",width=10,command=lambda:self.equals()).grid(row=4, column=4,columnspan=2)
  #lambda no hace ninguna falta en equals etc pq tienen nombre, Para los botones, si fuera, pq te evitas crear una 
  # funcion sumar que haga action('+') y otra restar que haga action('-')
  Button(master,text="=",width=8,command=self.equals).grid(row=4, column=4,columnspan=2)
